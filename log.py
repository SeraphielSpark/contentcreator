...
from flask_jwt_extended.exceptions import NoAuthorizationError, InvalidTokenError, ExpiredSignatureError
...
def get_jwt_identity_optional():
    ...
    except (NoAuthorizationError, InvalidTokenError, ExpiredSignatureError):
        return None
...
def generate():
    ...
    print(f"[DEBUG] /generate: Received post content: {post_content[:50]}...")
...
def respond():
    ...
    # Check for user identity (optional)
    # This will now correctly return None for expired/invalid tokens
    current_user_id = get_jwt_identity_optional()
    user = User.query.get(current_user_id) if current_user_id else None
    if user:
        print(f"[DEBUG] /respond: User identified - ID: {user.id}, Email: {user.email}")
    else:
        print("[DEBUG] /respond: No valid JWT token found, proceeding as anonymous.")
    ...
    Provide a helpful and concise response relevant to creators.
    """
    try:
        # --- *** REAL GenAI Call *** ---
        print("[DEBUG] /respond: Calling Google GenAI...")

        # Select the model (e.g., 'gemini-1.5-flash-latest' for chat)
        model = genai.GenerativeModel('gemini-1.5-flash-latest')

        # Send the chat prompt to the API
        ...
        history_id = None
        # If the user is logged in, save this chat to history
        if user:
            print("[DEBUG] /respond: Saving chat to history...")
            # Create a short title for the sidebar
            title = (prompt_content[:40] + '...') if len(prompt_content) > 40 else prompt_content
            ...
            try:
                search = SearchHistory(
                    ...
                    author=user # Use the user object directly
                )
                db.session.add(search)
                db.session.commit()
                history_id = search.id # Get the ID of the newly saved chat
                print(f"[DEBUG] /respond: History saved successfully with ID: {history_id}")
            except Exception as db_err:
                db.session.rollback() # Rollback in case of error
                print(f"[ERROR] /respond: Database error saving history: {str(db_err)}")
                # Don't fail the whole request, just log the error

        print(f"[DEBUG] /respond: Responding with result and history_id: {history_id}")
        return jsonify({"result": result, "history_id": history_id})
